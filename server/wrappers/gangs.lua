function loadGangAccount(gangid)
    ...
    QBCore.Functions.ExecuteSql(true, {['@gang'] = self.gid}, "SELECT * FROM `bank_accounts` WHERE `gangid` = @gang AND `account_type` = 'Gang'", function(query)
        if query[1] ~= nil then
    ...
    QBCore.Functions.ExecuteSql(true, {['@ac'] = self.accountnumber, ['@sc'] = self.sortcode, ['@gid'] = self.gid}, "SELECT * FROM `bank_statements` WHERE `account_number` = @ac AND `sort_code` = @sc AND `gangid` = @gid", function(state)
...
function createGangAccount(gang, startingBalance)
    ...
    QBCore.Functions.ExecuteSql(true, {['@gang'] = gang}, "SELECT * FROM `bank_accounts` WHERE `gangid` = @gang", function(checkExists)
