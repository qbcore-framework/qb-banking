...
function generatebusinessAccount(acc, sc, bid)
    ...
    QBCore.Functions.ExecuteSql(true, {['@acc'] = self.accountNumber, ['@sc'] = self.sortCode, ['@bid'] = self.bid}, "SELECT * FROM `bank_accounts` WHERE `account_number` = @acc AND `sort_code` = @sc AND `businessid` = @bid", function(bankAccount)
    ...
    rTable.getBankStatement = function(limit)
        ...
        local processed = false
        local statement
        QBCore.Functions.ExecuteSql(true, {['@bus'] = bankAccount[1].business, ['@acc'] = self.accountNumber, ['@sc'] = self.sortCode, ['@limit'] = resLimit, ['@bid'] = self.bid }, "SELECT * FROM `bank_statements` WHERE `account` = 'business' AND `business` = @bus AND `account_number` = @acc AND `sort_code` = @sc AND `businessid` = @bid LIMIT @limit", function(res)
            statement = res
            processed = true
        end)
        repeat Wait(0) until processed == true
...
function createbusinessAccount(accttype, bid, startingBalance)
    ...
    QBCore.Functions.ExecuteSql(true, {['@accttype'] = accttype, ['@bid'] = bid}, "SELECT * FROM `bank_accounts` WHERE `business` = @accttype AND `businessid` = @bid", function(checkExists)
