...
const getNuiCallbackUrl = (endpoint) => `https://qb-banking/atm/${endpoint}`

window.addEventListener("message", function (event) {   
    if(event.data.action == "open") {
        populateCurrentCards(event.data);
        $('#cardSelector').css({"display":"block"});
    } else if(event.data.action == "load-account") {
...
function closeBanking() {
    ...
    $.post("https://qb-banking/close-nui", JSON.stringify({}));
    currentLimit = null;
    clientPin = null;
    cardSelected = null;
    clientCid = null;
...
$( function() {
        ...
        if ($('#pinContainer').is(':visible')) {
            if (key.which >= 48 && key.which <= 57) { // 48 - 57 (0-9 keys)
                ...
                if (currentVal == clientPin) {
                    ...
                    $('#pinCode').val('');
                    $.post(getNuiCallbackUrl('get-account'), JSON.stringify({
                        cid: clientCid,
                        cardnumber: cardNumb
                    }));
                } else {
    ...
    $(document).on('click','[data-action=useCard]',function(){
        $.post(getNuiCallbackUrl('play-animation'), JSON.stringify({}));
    ...
    $(document).on('click','[data-action=removeCard]',function(){
        var selectedCard = $(this).data('card')
        $.post(getNuiCallbackUrl('remove-card'), JSON.stringify({
